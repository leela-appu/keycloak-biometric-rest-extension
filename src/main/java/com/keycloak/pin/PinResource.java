...
public class PinResource {
    ...
    public Response authenticate(@RequestBody PinValidateRequest request) {
        try {
            ...
            if (!credentialProvider.verifyPin(pinCredentialModel.getPinCredentialData().getSecurePin(), request.getPin())) {
    ...
    public Response delete(@RequestBody PinRemoveRequest request) {
        try {
            ...
            if (credentialProvider.deletePinCredential(existingUser, request.getDeviceId())) {
                String message = "Deleted secure pin for given device successfully";
                return Response.status(Response.Status.OK).entity(new ResponseData(Status.OK.getStatusCode(), message)).build();
            } else {
                String message = "Not found secure pin to delete";
                return Response.status(Response.Status.BAD_REQUEST).entity(new ResponseData(Status.BAD_REQUEST.getStatusCode(), message)).build();
            }
        } catch (NotAuthorizedException | ForbiddenException | VerificationException | CustomException e) {
            return resourceUtil.getErrorResponse(e, true);
        } catch (Exception e) {
